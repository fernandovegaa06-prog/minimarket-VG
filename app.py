...
st.set_page_config(
    page_title="Minimarket VG", page_icon="🛒", layout="centered"
)
...
def cargar_datos():
  inv_inicial = [
      ...
      {
          "Categoría": "Lácteos",
          "Producto": "Leche Gloria Azul (tarro)",
          "Precio Venta": 4.80,
          "Costo Compra": 4.10,
          "Stock": 3.0,
      },
      ...
      {
          "Categoría": "Bebidas",
          "Producto": "Cerveza Pilsen (Botella 650ml)",
          "Precio Venta": 8.50,
          "Costo Compra": 7.20,
          "Stock": 25.0,
      },
      ...
      {
          "Categoría": "Limpieza",
          "Producto": "Detergente Bolívar (1kg)",
          "Precio Venta": 11.50,
          "Costo Compra": 9.80,
          "Stock": 25.0,
      },
...
if (
    "inventario" not in st.session_state
    or "ventas" not in st.session_state
    or "gastos" not in st.session_state
):
...
if not st.session_state.autenticado:
  ...
  password_ingresada = st.text_input(
      "🔑 Ingrese su contraseña de acceso:", type="password"
  )
...
if menu == "🛒 Registrar Venta":
  ...
  producto_seleccionado = st.selectbox(
      "📦 Seleccione el producto:", lista_productos
  )
  prod_data = st.session_state.inventario[
      st.session_state.inventario["Producto"] == producto_seleccionado
  ].iloc[0]
  stock_actual, precio_venta, costo_compra, categoria_prod = (
      prod_data["Stock"],
      prod_data["Precio Venta"],
      prod_data["Costo Compra"],
      prod_data["Categoría"],
  )

  st.info(
      f"📂 Categoría: **{categoria_prod}** |  🏷️ Precio Venta: **S/"
      f" {precio_venta:.2f}**"
  )
  cantidad = st.number_input(
      f"⚖️ Cantidad (Stock disponible: {stock_actual}):",
      min_value=0.5,
  ...
  metodo_pago = st.radio(
      "💳 ¿Cómo pagó el cliente?", ["Efectivo", "Yape / Plin"], horizontal=True
  )

  total_cobrar, ganancia_estimada = (
      cantidad * precio_venta,
      cantidad * (precio_venta - costo_compra),
  )
  ...
  if st.button("✅ Cobrar y Registrar Venta", use_container_width=True):
    if stock_actual >= cantidad:
      ...
      st.success(
          "🎉 ¡Venta registrada con éxito y stock guardado permanentemente!"
      )
    else:
  ...
  with st.form("form_yape_delivery", clear_on_submit=True):
    monto_yape = st.number_input(
        "💰 Monto exacto que llegó por Yape/Plin (S/):",
        min_value=0.0,
        step=1.0,
        format="%.2f",
    )
    detalle_apuntes = st.text_area(
        "📝 Detalle de productos según tus apuntes o WhatsApp del cliente:",
        placeholder="Ej: S/ 30 de abarrotes, 1 aceite, 2 leches...",
    )
    ganancia_calculada = st.number_input(
        (
            "✨ Ganancia estimada aproximada para caja (S/) [Opcional - por"
            " defecto 20% del monto]:"
        ),
        min_value=0.0,
        value=0.0,
        step=0.50,
    )
    foto_voucher = st.file_uploader(
        "📸 Adjuntar captura de pantalla del voucher (Galería o cámara del"
        " celular)",
        type=["png", "jpg", "jpeg"],
    )

    btn_guardar_yape = st.form_submit_button(
        "✅ Registrar Ingreso Digital y Voucher"
    )
    ...
    if btn_guardar_yape:
      if monto_yape > 0:
        ganancia_final = (
            ganancia_calculada
            if ganancia_calculada > 0
            else (monto_yape * 0.20)
        )
        ahora = obtener_tiempo_peru()
        ...
        st.session_state.ventas.append({
            ...
            "Pago": "Yape / Plin",
            "Detalle": detalle_apuntes
            if detalle_apuntes
            else "Pago por Yape sin detalle",
        })
        guardar_ventas(st.session_state.ventas)

        st.success(
            f"🎉 ¡Yape de S/ {monto_yape:.2f} registrado y sumado a caja con"
            " éxito!"
        )
        ...
        if foto_voucher is not None:
          st.image(
              foto_voucher,
              caption="Voucher de Yape verificado y guardado",
              use_container_width=True,
          )
      else:
        st.error("❌ Por favor ingresa un monto válido mayor a 0.")

# OPCIÓN DE HISTORIAL Y BÚSQUEDA POR FECHAS
  ...
  if not st.session_state.ventas:
    st.info(
        "Todavía no hay ventas registradas en el historial. ¡Empieza a registrar"
        " ventas para verlas aquí!"
    )
  else:
    df_historial = pd.DataFrame(st.session_state.ventas)

    # Filtro de búsqueda por texto (producto o detalle)
    busqueda = st.text_input(
        "🔎 Buscar en el historial (Producto o detalle):", ""
    )
    if busqueda:
      df_historial = df_historial[
          df_historial["Producto"]
          .str.contains(busqueda, case=False, na=False)
          | df_historial["Detalle"]
          .str.contains(busqueda, case=False, na=False)
      ]

    st.dataframe(df_historial, use_container_width=True)

    # Botón para descargar el historial completo en CSV
    st.download_button(
        label="📥 Descargar Historial Completo en CSV",
        data=df_historial.to_csv(index=False).encode("utf-8"),
        file_name=(
            "historial_ventas_minimarket_vg_"
            f"{obtener_tiempo_peru().strftime('%Y-%m-%d')}.csv"
        ),
        mime="text/csv",
        use_container_width=True,
    )
  ...
  if not stock_critico.empty:
    st.warning(
        "⚠️ **¡Atención!** Los siguientes productos tienen stock menor o igual a"
        " 5 unidades. Considera reponerlos pronto:"
    )
    st.dataframe(stock_critico, use_container_width=True)
  else:
    st.success(
        "🎉 ¡Excelente! No hay ningún producto con stock crítico en este"
        " momento. Todo está abastecido."
    )
  ...
  inventario_df["Valor Total en Stock (S/)"] = (
      inventario_df["Stock"] * inventario_df["Precio Venta"]
  )
  ...
  with col2:
    st.metric(
        label="💵 Valor Total de Mercadería", value=f"S/ {valortotal_dinero:.2f}"
    )

  st.markdown("---")
  cat_elegida = st.selectbox(
      "📂 Filtrar Inventario por Categoría:",
      ["Todas"] + list(inventario_df["Categoría"].unique()),
  )
  inventario_filtrado = (
      inventario_df
      if cat_elegida == "Todas"
      else inventario_df[inventario_df["Categoría"] == cat_elegida]
  )
  ...
  prod_a_editar = st.selectbox(
      "🔍 Selecciona el producto a corregir:",
      st.session_state.inventario["Producto"].tolist(),
  )
  fila_prod = st.session_state.inventario[
      st.session_state.inventario["Producto"] == prod_a_editar
  ].iloc[0]

  st.info(
      f"📌 Estado actual de **{prod_a_editar}** ➔ Stock: {fila_prod['Stock']}"
      f" | Precio Venta: S/ {fila_prod['Precio Venta']:.2f}"
  )
  nuevo_stock = st.number_input(
      "📦 Stock exacto:", value=float(fila_prod["Stock"]), step=1.0
  )
  nuevo_precio = st.number_input(
      "🏷️ Precio de Venta (S/):",
      value=float(fila_prod["Precio Venta"]),
      step=0.10,
  )
  nuevo_costo = st.number_input(
      "📉 Costo de Compra (S/):",
      value=float(fila_prod["Costo Compra"]),
      step=0.10,
  )
  ...
  if st.button("💾 Guardar Cambios", use_container_width=True):
    st.session_state.inventario.loc[
        st.session_state.inventario["Producto"] == prod_a_editar,
        ["Stock", "Precio Venta", "Costo Compra"],
    ] = [nuevo_stock, nuevo_precio, nuevo_costo]
    guardar_inventario(st.session_state.inventario)
    st.success(
        f"✅ ¡Los datos de '{prod_a_editar}' han sido actualizados y guardados"
        " con éxito!"
    )
  ...
  total_ganancia_hoy = (
      sum(v["Ganancia"] for v in ventas_hoy) if ventas_hoy else 0.0
  )
  total_gastos_hoy = sum(g["Monto"] for g in gastos_hoy) if gastos_hoy else 0.0
  efectivo_hoy = (
      sum(v["Total"] for v in ventas_hoy if v["Pago"] == "Efectivo")
      if ventas_hoy
      else 0.0
  )
  yape_hoy = (
      sum(v["Total"] for v in ventas_hoy if v["Pago"] == "Yape / Plin")
      if ventas_hoy
      else 0.0
  )
  ...
  with col2:
    st.metric(
        label="💵 Efec / 📱 Yape", value=f"S/ {efectivo_hoy:.1f} / S/ {yape_hoy:.1f}"
    )
  with col3:
